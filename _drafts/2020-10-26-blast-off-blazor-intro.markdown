...
A couple weeks ago, I wrote about [deploying an Azure Static Web App with Blazor and Azure Functions](https://daveabrock.com/2020/10/13/azure-functions-static-apps-blazor). In that post, I talked about using the app I built as a base for an upcoming Blazor series. This is it!

We'll be building our site from the ground up with *no Blazor experience required*. All you'll need is a passing knowledge of C# and .NET. If you don't have that, no problem: [check out this Learn module and come back when you're ready](https://docs.microsoft.com/en-us/learn/paths/csharp-first-steps/).
...
With Blazor Web Assembly (which we'll be using in this project!), Blazor and its dependencies (including the .NET runtime!) is downloaded to the browser. The entire app executes on the browser UI thread! All app aseets, typically in `wwwroot`, are deployed as static files. 
...
I've ~~stolen~~ borrowed this diagram [from the Microsoft doc on the subject](https://docs.microsoft.com/en-us/aspnet/core/blazor/hosting-models?view=aspnetcore-3.1):
...
I've once again ~~stolen~~ borrowed this diagram [from the Microsoft doc on the subject](https://docs.microsoft.com/en-us/aspnet/core/blazor/hosting-models?view=aspnetcore-3.1):
...
As [written about previously](https://daveabrock.com/2020/10/13/azure-functions-static-apps-blazor), I've written an app called Blast Off With Blazor. You can see a live version at *[blastoffwithblazor.com](https://www.blastoffwithblazor.com/)* and [clone the GitHub repository](https://github.com/daveabrock/NASAImageOfDay).
...
Right now, it's super simple and super slow. We'll fix that in upcoming posts as we learn Blazor together. For now, let's look at the code.
...
Let's first look at the `Api` project as, for now, it's pretty cut-and-dry. As I said earlier, we're going to call the NASA APOD API. If you don't pass a date to the API it'll return the latest photo. I preferred to fetch a random one. 
...
There's actually quite a bit going on in the method signature. We start with defining an `HttpTrigger`, which means it'll execute when our app calls it. We pass in an `AuthLevel` - `Anonymous`, which means we don't pass in a Function-specific API key. The `Route` signifies we'll only be using GET calls, and the `Route` of `image` defines the route template (it'll respond to `api/image` calls). 

In the body of our method, we get our API key from our configuration (you'll learn how to do this in a later section), and call the API - passing in our key and the date. We'll also elect to receive HD images. Once we get a result back, we'll deserialize it so we can pass it back to the caller.
...
This `@page` attribute signifies a route template. In our case, the root of the app. That's all we need to know for now, but it can [get a lot more complicated](https://docs.microsoft.com/en-us/aspnet/core/blazor/fundamentals/routing?view=aspnetcore-3.1): you can even apply multiple route templates for a component.
...
Wait, what is that `http` variable? That is actually us injecting `HttpClient`, from the top of the file:
...
In the markup, you might be wondering about the HTML classes. With CSS for this project, [Chris Sainty](https://chrissainty.com/integrating-tailwind-css-with-blazor-using-gulp-part-1/) and Jon Hilton [sold me on Tailwind CSS](https://jonhilton.net/re-usable-components-with-blazor-and-tailwind-css/). Tailwind is what we call utility-first, allowing you to use a variety of classes that allows you to iterate easily. It takes a little bit to get used to, but it sure beats dropping in pre-built components that find their way everywhere ([hi, Bootstrap](https://getbootstrap.com/)).
